...
from mriforge.config.schemas.training.recoverability_vib import RecoverabilityVIBConfig


def test_defaults() -> None:
    c = RecoverabilityVIBConfig()
    assert c.beta >= 0.0
    assert c.lambda_recon >= 0.0
...
def test_rate_reduction_accepts_mean_per_dim() -> None:
    assert (
        RecoverabilityVIBConfig(rate_reduction="mean_per_dim").rate_reduction
        == "mean_per_dim"
    )
