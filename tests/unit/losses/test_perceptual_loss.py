...
class TestPerceptualLoss:
    ...
    @pytest.mark.slow
    @_vgg_skip
    def test_edge_nan_input_raises_rather_than_vanishing(self):
        """A non-finite prediction raises; it used to return 0.

        The guard returned ``0.0`` on NaN or Inf, so the loss vanished on
        exactly the batches that signal divergence and training carried on
        against a silent zero (corpus review, T0.14). The contract is now a
        raise, and this test pins the message rather than the old value.
        """
    ...
    def test_raises_actionable_message_when_torchvision_abi_mismatched(
        ...
        monkeypatch.setitem(sys.modules, "torchvision", _BrokenTorchvision("torchvision"))
        from spectramr.models.losses.perceptual_loss import PerceptualLoss
