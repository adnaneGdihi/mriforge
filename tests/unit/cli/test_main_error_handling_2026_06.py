"""Regression: ``mriforge`` main() now has a top-level error boundary.
...
message + exit 1 (full traceback only with ``-v`` / ``MRIFORGE_DEBUG``).
"""
...
from mriforge.cli import app, diagnostics


def _run_with_doctor_handler(monkeypatch, handler, argv):
    # main() does ``from mriforge.cli.diagnostics import run_doctor`` and binds it
...
def test_keyboard_interrupt_maps_to_130(monkeypatch):
    ...
    assert _run_with_doctor_handler(monkeypatch, boom, ["mriforge", "doctor"]) == 130
...
def test_generic_exception_maps_to_1(monkeypatch):
    ...
    assert _run_with_doctor_handler(monkeypatch, boom, ["mriforge", "doctor"]) == 1
...
def test_handler_systemexit_code_is_preserved(monkeypatch):
    ...
    with pytest.raises(SystemExit) as exc:
        _run_with_doctor_handler(monkeypatch, boom, ["mriforge", "doctor"])
    assert exc.value.code == 2


def test_clean_handler_return_passes_through(monkeypatch):
    assert _run_with_doctor_handler(monkeypatch, lambda _a: 0, ["mriforge", "doctor"]) == 0
