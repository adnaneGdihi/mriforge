...
# ---------------------------------------------------------------------------
# Lazy import of spectraMR modules (inside TestOneInput to avoid import-time
# crashes when the package is partially initialised)
# ---------------------------------------------------------------------------

def _import_spectramr():
    ...
    from spectramr.infrastructure.validation.config_health_checker import (  # noqa: PLC0415
        ConfigHealthChecker,
        HealthCheckReport,
    )
...
try:
    _TrainingSettings, _ConfigHealthChecker, _HealthCheckReport = _import_spectramr()
    _SPECTRAMR_AVAILABLE = True
except Exception as _e:
    logger.warning("spectraMR modules unavailable: %s", _e)
    _SPECTRAMR_AVAILABLE = False
