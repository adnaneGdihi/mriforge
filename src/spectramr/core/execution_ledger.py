...
from anywhere else in ``spectramr``. That is what lets ``config/settings.py``,
...
STRICT_ENV = "SPECTRAMR_LEDGER_STRICT"
...
_ACTIVE: contextvars.ContextVar[ExecutionLedger | None] = contextvars.ContextVar(
    "spectramr_execution_ledger", default=None
)
    ...
    def to_dict(
        ...
        Args:
            ...
            config_version: The audited config's own declared schema tier
                (:data:`~spectramr.config.schemas.base.CANONICAL_CONFIG_VERSION`
        ...
        **Three unrelated quantities in this artifact are spelled "version",
        ...
        AUTHOR field -- the revision of the ARM, not of the schema.

        468 of 647 ``inprogress/`` arms had it filled with a retired config tier
        (``6.0``/``6.1``); that residue was deleted from the corpus on
        2026-09-01, and ``test_metadata_version_is_not_a_retired_schema_tier``
        now ratchets it. Not caused by a framework default, though two of those
        did hardcode ``"6.0"`` and are also fixed: the production load path types
        ``TrainingSettings.metadata`` as a raw ``dict[str, Any]``, so neither
        default ever reached an arm. The arms declared the tier in their own
        YAML, inherited from a template.
...
def diff_declared_vs_resolved(
    ...
    *,
    ...
    path_prefix: str = "",
    site: str = "spectramr.config.settings._finalize_from_dict",
    _depth: int = 0,
) -> None:
