...
``spectramr.models`` is lazy (see ``docs/cli_startup_budget.rst``).
...
``cli/app.py`` imports ``spectramr.core.env`` to build the parser -- and importing
...
``spectramr --help`` (issue #1130).

Nothing is lost for real consumers: importing a submodule always executes the
parent ``__init__``, so ``from spectramr.core.metrics.registry import ...`` still
...
def __getattr__(name: str):
    ...
    ``from spectramr.core import <submodule>`` keeps working for all of them:
