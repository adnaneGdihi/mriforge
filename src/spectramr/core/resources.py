...
* **Decisions must not.** A caller that *acts* on ``usable_cores`` has to treat
  ...
  non-negotiable 3. :func:`spectramr.core.topology.resolve_run_topology` is the
...
def allocated_gpus_per_node(
    ...
    :func:`spectramr.core.topology.resolve_run_topology`, which stamps it into
...
def visible_gpu_count() -> int | None:
    ...
    **Visible, not allocated.** The two coincide on a plain ``--gpus=N`` job and
    ...
    Torch is imported lazily, exactly as :mod:`spectramr.core.compute_device` does
...
def cgroup_cpu_quota() -> float | None:
    ...
    Docker/k8s (and the ``spectramr launch`` docker backend) cap CPU via a cgroup
