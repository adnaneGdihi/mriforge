...
Registration: every module under ``spectramr.core.metrics`` is walked on
...
#: An in-repo (``spectramr.*``) import failure is NEVER recorded here — it is
...
_PACKAGE_NAME = __name__  # "spectramr.core.metrics"
...
def _record_or_raise_walk_import_error(module_name: str, exc: ImportError) -> None:
    ...
    * An **in-repo** failure (``exc.name`` starts with ``spectramr`` — or is
    ...
    * A **third-party** root cause (torchmetrics, piq, pyradiomics, …) is a
    ...
    """
    root = exc.name or ""
    if not root or root == "spectramr" or root.startswith("spectramr."):
        raise exc
    ...
    logger.warning(
        ...
        "Recorded in spectramr.core.metrics.MISSING_OPTIONAL_DEPS. "
        "Audit 13 F11.",
        module_name,
        root,
        exc,
...
# SPECTRAMR_PLUGINS modules. spectramr.plugins is stdlib-only — no layer violation.
# A bad SPECTRAMR_PLUGINS token raises (fail-fast, pitfall #15).
from spectramr.plugins import discover_plugins as _discover_plugins  # noqa: E402

_discover_plugins("spectramr.metrics")
