...
Example:
    >>> from spectramr.config.schemas.enums import TrainingMode, GANLossType
...
# each ``Regime`` live in ``spectramr.domain.workflows``.
#
...
class Task(str, Enum):
    ...
    :class:`~spectramr.domain.workflows.profiles.WorkflowProfile`, pinned by
