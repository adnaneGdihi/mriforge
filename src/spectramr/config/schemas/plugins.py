...
(:func:`spectramr.plugins.discover_plugins`) so a user can register custom
models / losses / metrics / strategies that live **outside** the
``spectramr`` source tree — in their own pip-installed package or a standalone
...
``SPECTRAMR_PLUGINS`` env var: both name dotted import paths to import (which
...
class PluginsConfigSchema(BaseModel):
    ...
    Attributes:
        enabled: Master switch. Discovery layers (entry-points + the
            ``SPECTRAMR_PLUGINS`` env var) run regardless; this flag gates the
