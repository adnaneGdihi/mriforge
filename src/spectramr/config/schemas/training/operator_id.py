...
* :class:`OperatorIDConfig` — the nested ``training.operator_id`` block
  (also referenced by :class:`spectramr.config.schemas.training.base.TrainingStrategyConfigSchema`).
...
# spectramr.infrastructure.physics.magnus_exponential — kept in sync with the
