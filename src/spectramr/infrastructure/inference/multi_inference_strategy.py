...
class MultiInferenceStrategy(BaseInferenceStrategy):
    ...
    def __init__(
        ...
        # ``infer_single`` below never calls ``BaseInferenceStrategy.infer``,
        # where the predict-time projection runs, and takes no mask or
        # measurement. Accepting the knob here would leave it inert (#16).
        if self.predict_dc is not None:
            from spectramr.domain.exceptions import ConfigurationError
            from spectramr.infrastructure.inference.predict_data_consistency import KNOB

            raise ConfigurationError(
                f"{KNOB} is not supported by multi-stage inference: "
                "MultiInferenceStrategy.infer_single bypasses the base infer() hook "
                "and carries no mask or measurement. Turn the knob off for this arm."
            )
