...
1. domain chain ``data -> model.input`` -- OWNED by
   ``config_health_checker.check_data_model_compatibility`` since 2026-09-03: it
   folds the declared adapter chain and knows which strategies adjoint k-space to
   the image domain internally; the rule that lived here compared the loader's
   domain against ``input_domain`` with neither, so it was a weaker second owner
   (rejected 11 arms the health check accepts) and was deleted, not kept as
   defence in depth. The ``model.output -> loss`` leg is likewise the health
   checker's (``loss_domain_consistency`` / ``model_loss_output_domain``).
...
from spectramr.domain.entities.experiment_context import ResolvedExperimentContext
