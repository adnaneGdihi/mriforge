...
from spectramr.infrastructure.logging import MetricsTracker  # noqa: F401
...
def initialize_profiler() -> None:
    ...
    try:
        import spectramr.models.analysis.profile_training as profile_training  # type: ignore
...
def _resolve_gpu_memory_fraction() -> float:
    ...
    ``SPECTRAMR_GPU_MEMORY_FRACTION`` (registered in ``core/env.py``) now
    ...
    """
    raw = os.environ.get("SPECTRAMR_GPU_MEMORY_FRACTION")
    if raw is None:
    ...
    try:
        ...
        raise ValueError(
            f"SPECTRAMR_GPU_MEMORY_FRACTION must be a float in (0, 1], got {raw!r}"
        ) from None
    if not (0.0 < fraction <= 1.0):
        raise ValueError(f"SPECTRAMR_GPU_MEMORY_FRACTION must be in (0, 1], got {fraction}")
    return fraction
...
def initialize_device() -> torch.device:
    ...
    try:
        ...
        if cuda_available:
            ...
            # SPECTRAMR_GPU_MEMORY_FRACTION (default 0.85), validated + logged.
            ...
            logging.info(
                ...
                f"via SPECTRAMR_GPU_MEMORY_FRACTION)",
            )
...
def get_default_device() -> "torch.device":
    ...
    importer (CLI, ``spectramr audit``, the test suite), despite having zero
