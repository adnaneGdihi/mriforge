...
class NUFFTTrajectoryCalibrator(nn.Module):
    ...
    def marker_image(self, kspace: torch.Tensor, delays: torch.Tensor) -> torch.Tensor:
        """Magnitude image of ``kspace`` after the delay correction, in the image frame
        the marker mask and prior are drawn in.

        ``kspace`` is centred (``compute_phase_correction`` builds its ramp on the
        ``fftshift``-ed frequency grid), so the inverse transform is ``ifft2c``.
        Until 2026-09-03 this was a raw ``torch.fft.ifft2``, whose output is the
        image circularly shifted by half a field of view: the marker mask then
        selected pixels half a FOV away from the marker, and the delay search
        optimised the wrong region (non-negotiable 2).
        """
        from spectramr.infrastructure.physics.fft_ops import ifft2c

        correction = self.compute_phase_correction(delays)
        return ifft2c(kspace * correction).abs()
    ...
    def calibrate(
        ...
        for _it in range(num_iters):
            optimizer.zero_grad()
            image_mag = self.marker_image(kspace, delays)
