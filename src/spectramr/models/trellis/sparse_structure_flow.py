"""Sparse Structure Flow model adapted for spectraMR integration.
...
from spectramr.models.blocks.trellis_transformer import AbsolutePositionEmbedder
from spectramr.models.blocks.trellis_utils import convert_module_to_f16, convert_module_to_f32
...
class SparseStructureFlowModel(nn.Module):
    ...
    def __init__(
        ...
        # ``trellis_ATTENTION``, not ``trellis_transformer``. The class is defined at
        # ``blocks/trellis_attention.py:555`` and named in its ``__all__``;
        # ``trellis_transformer`` has never had it, so this raised at CONSTRUCTION --
        # the model registered fine and could not be instantiated. Being a
        # function-local import, no import-time check could see it.
