...
# CLI cold-start (~6 s on ``spectramr --help``). Since 2026-07-02 it is a
...
# ``__getattr__`` below keeps ``spectramr.models.sota_registry`` resolvable
...
def __getattr__(name: str):
    ...
    if name == "sota_registry":
        import spectramr.models.sota_registry as _sota_registry

        return _sota_registry
    raise AttributeError(f"module 'spectramr.models' has no attribute {name!r}")
