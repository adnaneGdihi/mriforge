...
class WaveletConvND(nn.Module):
    ...
    @staticmethod
    def _forward_daubechies(x):
        """A compact-support approximation of a Daubechies-like wavelet.

        This is a smooth, compact-ish approximation built from a polynomial
        multiplied by a Gaussian envelope. It is not the exact discrete
        Daubechies construction but behaves similarly for continuous inputs.
        """
        # polynomial shaped to have oscillation and compact-like decay
        # x shape: (batch, channels, length, ...)
        # create a low-order polynomial times gaussian envelope
        poly = 1.0 - 6.0 * (x**2) + (x**4)
        env = torch.exp(-0.5 * (x**2))
        wavelet = poly * env
        # zero-mean along the coordinate axis to mimic wavelet oscillation
        # assume the spatial axis is dim=2 (matches other methods)
        try:
            mean = wavelet.mean(dim=2, keepdim=True)
    ...
    @staticmethod
    def _forward_uwt(x):
        """Undecimated Wavelet Transform (simple multi-scale undecimated sum).

        This constructs a multi-scale, undecimated wavelet-like response by
        summing Mexican-hat responses at different scales. It preserves the
        input sampling (no decimation) and is compatible with the module's
        expectation of a same-shaped wavelet tensor.
        """
        scales = [1.0, 2.0, 4.0]
        out = torch.zeros_like(x)
        for s in scales:
            out = out + WaveletConvND._forward_mexican_hat(x / s)
        # normalize energy so larger number of scales doesn't blow up
    ...
    def forward(self, x):
        ...
        forward method for WaveletConvND.
        ...
        if self.wavelet_type == "mexican_hat":
            ...
            wavelet = self._forward_daubechies(x_scaled)
        elif self.wavelet_type in ("uwt", "undecimated"):
            wavelet = self._forward_uwt(x_scaled)
...
class WaveletConvNDFastPlusOne(WaveletConvND):
    """WaveletConvNDFastPlusOne class."""

    def __init__(
        self,
        conv_class,
        conv_class_d_plus_one,
        input_dim,
        output_dim,
        kernel_size,
        padding=0,
        ...
        Args:
            conv_class (Any): Description.
            conv_class_d_plus_one (Any): Description.
            input_dim (Any): Description.
            output_dim (Any): Description.
            kernel_size (Any): Description.
            padding (Any): Description.
        ...
        kernel_size_plus = (
            ...
            else (input_dim,) + (kernel_size,) * ndim
        )
        ...
        dilation_plus = (
            (1,) + dilation if isinstance(dilation, tuple) else (1,) + (dilation,) * ndim
        )
...
class WavKANConvNDLayer(nn.Module):
    ...
    def __init__(
        ...
        **norm_kwargs,
        ...
        Args:
            conv_class (Any): Description.
            conv_class_plus1 (Any): Description.
        ...
        self.groups = groups
        self.ndim = ndim
        # Filter ``norm_kwargs`` to only the kwargs accepted by the norm
        # constructor. The caller (RefinedKANUNet → DoubleConvWithKAN →
        # WavKANConv*DLayer) forwards the full YAML ``model_kwargs`` blob,
        # which can include KAN-specific options (``grid_size``,
        # ``spline_order``, ``scale_noise``, ``scale_base``,
        # ``grid_update_freq``, ``grid_range``, ``grid_update_decay``,
        # ``base_filters``, ``features``, ``bottleneck_only`` …).
        # Pre-2026-05-15 these landed on ``_BatchNorm.__init__()`` and
        # raised ``TypeError: got an unexpected keyword argument
        # 'grid_size'``. Audit ref: ``smoke_audit_20260514.md`` E13/F8.
        import inspect as _inspect

        _norm_sig = _inspect.signature(norm_class)
        _valid_norm_keys = {
            k
            for k, p in _norm_sig.parameters.items()
            if p.kind in (p.POSITIONAL_OR_KEYWORD, p.KEYWORD_ONLY)
        }
        self.norm_kwargs = {k: v for k, v in norm_kwargs.items() if k in _valid_norm_keys}
        ...
        self.layer_norm = nn.ModuleList(
            [norm_class(output_dim // groups, **self.norm_kwargs) for _ in range(groups)],
        )
    ...
    def forward(self, x):
        ...
        forward method for WavKANConvNDLayer.
