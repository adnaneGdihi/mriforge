...
class BottleNeckKAGNConvNDLayer(nn.Module):
    """BottleNeckKAGNConvNDLayer class."""
    ...
    def __init__(
        ...
        **norm_kwargs,
        ...
        inner_dim = int(
            max(
                (input_dim // groups) / dim_reduction,
    ...
    def forward(self, x):
        ...
        forward method for BottleNeckKAGNConvNDLayer.
...
class BottleNeckKAGNConv3DLayer(BottleNeckKAGNConvNDLayer):
    """BottleNeckKAGNConv3DLayer class."""
...
class BottleNeckKAGNConv2DLayer(BottleNeckKAGNConvNDLayer):
    """BottleNeckKAGNConv2DLayer class."""
...
class BottleNeckKAGNConv1DLayer(BottleNeckKAGNConvNDLayer):
    """BottleNeckKAGNConv1DLayer class."""
...
class KAGNMoE(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            dropout (float): Description.
            ndim (int): Description.
            noisy_gating (Any): Description.
            k (Any): Description.
            pregate (bool): Description.
        """
        super().__init__()
        ...
        self.w_gate = nn.Parameter(
            torch.zeros(input_dim, num_experts),
            requires_grad=True,
        )
    ...
    def forward(self, x, loss_coef=1e-2):
        ...
        forward method for KAGNMoE.

        Executes PyTorch tensor operations.

        Args:
            x (torch.Tensor, shape (B, C, H, W) [inferred]): Expected input tensor.
            loss_coef (torch.Tensor, shape (B, C, H, W) [inferred]): Expected input tensor.
...
class MoEBottleNeckKAGNConvND(nn.Module):
    ...
    def __init__(
        ...
        **norm_kwargs,
        ...
        self.w_gate = nn.Parameter(
            torch.zeros(self.inner_dim * groups, num_experts),
            requires_grad=True,
        )
        self.w_noise = nn.Parameter(
            torch.zeros(self.inner_dim * groups, num_experts),
    ...
    def forward(self, x, train=True, loss_coef=1e-2):
        ...
        Args:
            x (Any): Description.
            train (Any): Description.
            loss_coef (Any): Description.
        Returns:
            Any: Description.

        forward method for MoEBottleNeckKAGNConvND.

        Executes PyTorch tensor operations.

        Args:
            x (torch.Tensor, shape (B, C, H, W) [inferred]): Expected input tensor.
            train (torch.Tensor, shape (B, C, H, W) [inferred]): Expected input tensor.
            loss_coef (torch.Tensor, shape (B, C, H, W) [inferred]): Expected input tensor.
        ...
        output = []
        for group_ind, (_xb, _xe) in enumerate(
            zip(bases, torch.split(y, self.inner_dim, dim=1), strict=False),
        ):
...
class MoEBottleNeckKAGNConv3DLayer(MoEBottleNeckKAGNConvND):
    """MoEBottleNeckKAGNConv3DLayer class."""
...
class MoEBottleNeckKAGNConv2DLayer(MoEBottleNeckKAGNConvND):
    """MoEBottleNeckKAGNConv2DLayer class."""
...
class MoEBottleNeckKAGNConv1DLayer(MoEBottleNeckKAGNConvND):
    """MoEBottleNeckKAGNConv1DLayer class."""
