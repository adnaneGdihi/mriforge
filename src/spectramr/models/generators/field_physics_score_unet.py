...
:class:`~spectramr.models.generators.field_guided_score_unet.FieldGuidedScoreUNet` (a field-guided
...
* **Unconditional** (no source image, ``cond_channels=0``): it models ``p(x | b)`` and is therefore
  ...
  :class:`~spectramr.models.diffusion.posterior_langevin.PosteriorLangevinEnsemble` / DPS — as a
  ...
  the shipped :class:`~spectramr.models.diffusion.field_score_prior_adapter.FieldScorePriorAdapter`.
* **Physics-embedded conditioning**: the field enters through the Bottomley ``T1(B0)`` relaxation
  dispersion (:class:`~spectramr.models.blocks.physics_field_embedding.PhysicsFieldEmbedding`) rather
