...
names resolve through :class:`~spectramr.models.losses.registry.LossRegistry`
...
- ``l1``  → :class:`~spectramr.models.losses.standard_losses.L1Loss`
  (``nn.L1Loss`` subclass) ≡ ``F.l1_loss``.
- ``l2``  → :class:`~spectramr.models.losses.standard_losses.MSELoss`
  (``nn.MSELoss`` subclass) ≡ ``F.mse_loss``.
- ``huber`` → :class:`~spectramr.models.losses.diffusion_losses.HuberLoss`,
