...
(``spectramr/models/init_registry.py`` — see the dated 2026-07-02 walk-list
...
``infrastructure/physics/`` to ``spectramr/models/physics/`` as part of the
same pass.
...
def register_sota_models() -> None:
    ...
    ``spectramr.models.init_registry.populate_model_registry()``; the roster is
