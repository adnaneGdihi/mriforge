...
import machinery resolves ``spectramr.cli`` to ``src/cli/__init__.py``
whenever both targets exist, leaving the original file's symbols
(``train``, ``predict``, ``list_features``, …) unreachable.

The fix relocates the original module to :mod:`spectramr.cli.app` and re-
exports every public symbol from here so existing call sites
(``from spectramr.cli import list_features``, ``python -m spectramr.cli train …``)
continue to work without changes.
"""

# Re-export the full app surface so tests / scripts that do
# ``from spectramr.cli import list_features`` continue to resolve correctly.
# The wildcard pulls in every name not prefixed with ``_``; ``main``
# and the subcommand functions all qualify.
from spectramr.cli import regulatory
from spectramr.cli.app import *  # noqa: F403
