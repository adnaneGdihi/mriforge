...
from spectramr.infrastructure.optimization.tto import TestTimeOptimizer
from spectramr.infrastructure.physics.fft_ops import ifft2c
from spectramr.models.gaussian_splatting.medgs import MedGS
from spectramr.models.generators.latent_gaussian_diffusion import LatentGaussianDiffusion
...
class PGGSReconstructionPipeline(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        if device is None:
            # Resolved through the one device policy (non-negotiable 9b), not a
            # `"cuda" if available else "cpu"` default evaluated at import time.
            from spectramr.core.compute_device import resolve_torch_device

            device = torch.device(
                resolve_torch_device(None, pipeline="pggs", source="PGGSReconstructionPipeline").device
            )
