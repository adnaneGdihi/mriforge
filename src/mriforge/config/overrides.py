...
``apply_overrides`` and ``_parse_value`` used to live in ``mriforge.main`` (the
...
import rightward. ``mriforge.main`` re-exports both for backward compatibility.
"""
...
def apply_overrides(settings: Any, overrides: list[str]) -> Any:
    ...
    * The 2026-05-29 ``model_domain`` round-trip break, worked around at the
    ...
    :meth:`~mriforge.config.settings.TrainingSettings.get_validated_snapshot`,
