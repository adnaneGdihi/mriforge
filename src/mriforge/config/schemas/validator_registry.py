...
def _validate_epochs(config: dict[str, Any]) -> list[str]:
    """Validate that epochs is >= 1."""
    messages = []

    # Check both optimization.epochs and training.epochs (different schema versions)
    epochs = config.get("optimization", {}).get("epochs")
    if epochs is None:
        epochs = config.get("training", {}).get("epochs")

    # If max_iterations is set, epochs is optional
    max_iterations = config.get("training", {}).get("max_iterations")
    if max_iterations is not None and max_iterations > 0:
        return messages

    if epochs is None or epochs < 1:
        messages.append(f"epochs must be >= 1, got {epochs} (or specify training.max_iterations)")
...
def _validate_kspace_model_config(config: dict[str, Any]) -> list[str]:
    """Validate k-space model configuration.

    This is a simplified version of the domain-specific validation.
    """
    messages = []
    ...
    # Validation: Metrics should be in image domain
    if metrics_domain != "image":
        messages.append(
            "K-space models should calculate metrics in 'image' domain. "
            f"Current domain: {metrics_domain or 'not specified'}. "
            "SSIM/PSNR in k-space are scientifically invalid."
        )
...
def _validate_loss_weights_positive(config: dict[str, Any]) -> list[str]:
    ...
    """
    from mriforge.config.validation_constants import LOSS_WEIGHT_RANGES
...
def _validate_training_mode_compatibility(config: dict[str, Any]) -> list[str]:
    ...
    ``mriforge.infrastructure.validation.config_validation``
    ...
    """
    from mriforge.config.validation_constants import TRAINING_MODE_CONSTRAINTS
    ...
    # ``mriforge.infrastructure.validation.config_validation``. TRAINING_MODE_CONSTRAINTS
