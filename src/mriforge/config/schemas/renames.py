...
#: under `tests/`, `src/mriforge/config/presets/` or `scripts/`, which the default
#: invocation never opens::
#:
#:     python scripts/ci/check_no_legacy_config_keys.py
#:     python scripts/ci/check_no_legacy_config_keys.py tests src/mriforge/config/presets scripts
#:
...
RENAMES: dict[str, RenameRecord] = _index(
    [
        ...
        *_folds(
            ...
            "reader in src/mriforge, already carried in KNOWN_UNCONSUMED.",
        ),
        *_folds(
            [
                # k-space scaling
