...
:class:`~mriforge.config.key_reachability_collect._FileCollector` over
...
sits in ``src/mriforge/config/`` -- an invariant
...
from mriforge.config.key_reachability_collect import _FileCollector
from mriforge.config.key_reachability_model import _Index

#: ``src/mriforge`` -- the only tree a read site may come from.
