...
class IndexBuilder:
    ...
    def build_nifti_index(config: DataConfig, split: str) -> list:
        ...
        if is_split_dir:
            ...
            if holdout_site:
                ...
                # Deterministic FILE-level split (never slice-level, to avoid
                # leaking slices of one volume into both train and val).
    ...
    def _build_paired_nifti_index(config, root: Path, split: str, is_split_dir: bool) -> list:
        ...
        if not is_split_dir and not holdout_site:
            ...
            index = val_index if split == "val" else train_index
    ...
    def _build_bids_index(config, root: Path, split: str) -> list:
        ...
        # If no participants.tsv, do manual split
        if allowed_subjects is None:
            # Deterministic file-level split (train-only when validation_split=0).
            train_files, val_files = split_index(files, config.split.validation_fraction)
    ...
    def load_paired_bids_manifest(
        ...
        split :
            ``"train"`` or ``"val"``.
        config :
            A :class:`~mriforge.config.schemas.data.DataConfigSchema` or
            compatible object exposing a ``pairing`` block -- i.e. one
            carrying a :class:`~mriforge.config.schemas.data.DataPairingConfigSchema`.
    ...
    def _resolve_manifest_paths(records: list[dict], *, data_root: str = "") -> list[dict]:
        ...
        Uses :class:`~mriforge.data.metadata.path_resolver.PathResolver` which
