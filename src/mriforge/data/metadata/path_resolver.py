...
The project root is determined by (in priority order):
    ...
    2. ``MRIFORGE_DATA_ROOT`` environment variable
    3. Walking up from this file to find ``pyproject.toml``

On a cluster, set ``PROJECT_ROOT`` (or ``MRIFORGE_DATA_ROOT``) to your data
...
def get_project_root() -> Path:
    """Return the project root, checking PROJECT_ROOT / MRIFORGE_DATA_ROOT first."""
    env_root = os.environ.get("PROJECT_ROOT") or os.environ.get("MRIFORGE_DATA_ROOT")
    if env_root:
...
# Extend via the MRIFORGE_LEGACY_ABS_PREFIXES env var (colon-separated) if you
# have your own cluster mount roots to handle.
_LEGACY_ABS_PREFIXES = [
    p for p in os.environ.get("MRIFORGE_LEGACY_ABS_PREFIXES", "").split(":") if p
]
