...
``RandomBiasField``, so :mod:`mriforge.data.transforms.augmentation_factory`
uses those directly.  It does **not** ship brightness/contrast jitter, and the
project's own :class:`~mriforge.data.transforms.realistic_degradations.RicianNoise`
and :class:`~mriforge.data.transforms.realistic_degradations.MotionBlur3D` are
...
class RandomRicianNoise(tio.IntensityTransform):
    ...
    def __init__(self, noise_level: float, p: float = 1.0):
        ...
        from mriforge.data.transforms.realistic_degradations import RicianNoise
...
class RandomMotionBlur(tio.IntensityTransform):
    ...
    def __init__(
        ...
        from mriforge.data.transforms.realistic_degradations import MotionBlur3D
