...
class FastMRIH5Strategy(BaseIOStrategy):
        ...
        with h5py.File(path, "r", swmr=True) as f:
            ...
            if "kspace" in f:
                if slice_idx is not None:
                    # Lazy read: only load specific slice
                    # Shape is typically (Slices, Coils, H, W) or (Slices, H, W)
                    kspace_np = f["kspace"][slice_idx]
                    # Add batch dim if single slice? No, usually expect (C, H, W) or (H, W)
                    # existing code expects full volume?
                    # If we load single slice, we get (Coils, H, W).
                else:
                    kspace_np = f["kspace"][()]
    ...
    def load_reference_volume(path: str | Path) -> dict[str, Any]:
        ...
        affine. See ``mriforge.data.nifti_export``.
        """
        import h5py
    ...
    def load_torchio_tensor(
        ...
        ``mriforge.data.metadata.index_builder`` to keep the h5py read inside
...
class TwixStrategy(BaseIOStrategy):
    ...
    decoding to the dependency-free :mod:`mriforge.data.twix` decoder (the SSOT, also used
    ...
    def load(self, path: str, metadata: dict | None = None) -> dict[str, Any]:
        ...
        from mriforge.data.twix import read_twix
