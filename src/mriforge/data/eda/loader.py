...
from mriforge.data.eda.catalog import _SHARED_CONTAINERS, resolve_scan_dir
from mriforge.data.io_strategies import IOStrategyFactory, load_tensor_from_file
from mriforge.data.profiler import profile_dataset
from mriforge.infrastructure.physics.fft_ops import coil_combine_rss, ifft2c
...
def _scan_dir(entry) -> Path | None:
    ...
    Delegates to :func:`mriforge.data.eda.catalog.resolve_scan_dir` (the single SSOT used by the
...
def _cfl_central_slice(path: Path) -> torch.Tensor | None:
    ...
    try:
        ...
        from mriforge.data.io_strategies import BartCflStrategy
