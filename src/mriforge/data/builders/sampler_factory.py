...
``"grid"`` (handled by :mod:`mriforge.data.builders.inference_tiling`),
nor ``"temporal_*"`` (Phase 4c — temporal samplers live in
:mod:`mriforge.data.builders.temporal_sampler`).
"""
...
def build_patch_sampler(sampler_config, patch_size: tuple) -> Any:
    ...
    if stype not in PATCH_SAMPLER_TYPES:
        raise ValueError(
            ...
            "'grid', call mriforge.data.builders.inference_tiling. For "
            "'temporal_*', call mriforge.data.builders.temporal_sampler "
            "(Phase 4c)."
        )
