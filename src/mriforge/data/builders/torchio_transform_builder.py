...
# ``infrastructure.physics.trajectories`` directly: ``mriforge.data`` -> infrastructure
...
from mriforge.data.transforms.non_cartesian import (
...
class TorchIOTransformConfig:
    ...
    def from_training_config(config) -> "TorchIOTransformConfig":
        ...
        # New robust normalization config
        ...
        if accel_config:
            # Use max_acceleration for dataloader (represents target undersampling)
            # Training strategy will apply dynamic masking with varying acceleration
            # But dataloader should provide the target undersampled data
            base_accel = accel_config.base_acceleration
            max_accel = accel_config.max_acceleration

            # For validation: use max_acceleration if base is 1.0 (no identity masking)
            # Otherwise use base_acceleration
            if base_accel > 1.0:
...
class TorchIOTransformBuilder:
    ...
    def build_train_transforms(config: TorchIOTransformConfig) -> tio.Compose:
        ...
        if config.multi_echo_enabled:
            from mriforge.data.transforms.multi_echo import BundleMultiEcho
        ...
        from mriforge.data.transforms.kspace_coil_transforms import (
        ...
        # [CRITICAL FIX] Mutual Exclusion: Do NOT apply image normalization if k-space normalization is active.
        # Image norms (RescaleIntensity, ZNorm) assume magnitude images and often clamp/shift values,
        # which destroys complex k-space (negative values, phase).
        if not config.normalize_kspace:
            # ONE image normalizer, resolved by ImageNormalizationSpec and
            # applied by ImageNormalizationTransform.
            #
            # This replaces a three-branch tio.ZNormalization/RescaleIntensity
            # dispatch, and it is not a tidy-up. `ContrastAwarePairedDataset`
            # normalizes `input`/`target` itself and then calls
            # `self.transform(subject)` on its LAST line, so the chain built
            # here ran second on an already-normalized tensor — 87 arms
            # normalized twice, with the two passes disagreeing on percentile
            # (99.5 vs 99), noise floor, and clamping. The dataset side is now
            # off and this is the only pass (#760, the image-domain twin of
            # #571).
            #
            # The percentile branch also stops CLIPPING. `RescaleIntensity(
            # percentiles=(0, p))` clips at the percentile before rescaling,
            # while the comment it carried described "divide by the 99th";
            # `normalize_percentile` divides and clamps only when asked (B5).
            from mriforge.data.transforms.normalization import (
            ...
            if spec.enabled:
                ...
                logger.info(
                    ...
                    "(normalization_type='none' on a dataset_type that never "
                    "normalized internally)."
                )
        else:
            if config.normalization_type != "none":
                logger.info(
                    f"[NORMALIZATION] Image normalization '{config.normalization_type}' SKIPPED "
                    f"because normalize_kspace=True. Preserving complex k-space values."
                )
    ...
    def _append_image_undersampling(transforms: list, config: TorchIOTransformConfig) -> None:
        ...
        if not getattr(config, "image_undersampling", False):
            return
        from mriforge.data.transforms.image_undersampling import (
    ...
    def build_val_transforms(config: TorchIOTransformConfig) -> tio.Compose:
        ...
        # three-branch dispatch, and they had already drifted: the train copy
        # warned on an unknown `normalization_type` while this one had no
        # unknown branch at all, so a val chain silently applied nothing where
        # the train chain at least said something. One spec, both chains.
        if not config.normalize_kspace:
            from mriforge.data.transforms.normalization import (
                ImageNormalizationSpec,
                ImageNormalizationTransform,
            ...
            if config.normalization_type != "none":
                logger.info(
                    f"[VAL-NORMALIZATION] Image normalization '{config.normalization_type}' SKIPPED "
                    f"because normalize_kspace=True."
                )
    ...
    def _append_coordinate_grid_transform(transforms: list, config: TorchIOTransformConfig) -> None:
        ...
        if config.coordinate_emission_config is not None:
            from mriforge.data.transforms.coordinate_grid import CoordinateGridTransform
