...
_IMAGE_KEYS = ("input", "target", "sources")
...
class _VolumeLRU:
    ...
    def _check_footprint(self, volume: torch.Tensor) -> None:
        ...
        raise ValueError(
            ...
            "needs 5); under that floor every sample evicts the volume the next "
...
def mrixfields_pairing_feasibility(
    ...
    *,
    ...
    (and ``_build_multi_source`` / ``_build_multi_contrast``) and are kept there
...
class MRIxFieldsPairedDataset(torch.utils.data.Dataset):
    ...
    Args:
        ...
        slice_mode: which slice(s) each volume serves. ``"all_slices"`` (default, the
            ...
            ``[C,H,W,D]`` volume (``spatial_dims: 3`` arms); it is NOT supported with
            ``multi_source`` / ``multi_contrast`` (that would emit a 5-D ``sources`` stack
            no strategy consumes) and raises for those.
    ...
    def __init__(
        ...
        *,
        ...
        if slice_mode == "volume" and pairing_policy in (
            "multi_source",
            "multi_contrast",
        ):
    ...
    def __len__(self) -> int:
        ...
        if self._policy in ("multi_source", "multi_contrast"):
    ...
    def _containers(self) -> list[Any]:
        ...
        return self._tuples if self._policy in ("multi_source", "multi_contrast") else self._pairs

    def container_volume_paths(self) -> list[frozenset[str]]:
        """Volume paths each container reads, indexed by container.

        The input :class:`~mriforge.data.samplers.VolumeBlockedSliceSampler` needs to order
    ...
    def _build_index_map(self) -> list[tuple[int, int]]:
        ...
        for ``multi_source`` / ``multi_contrast`` — so all policies gain per-slice
