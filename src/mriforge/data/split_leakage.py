...
under ``src/mriforge/data/``), so the raw scan lives here and the infrastructure
...
splits partition a *single* index via :func:`mriforge.data.split_utils.split_index`
...
__all__ = ["SplitLeakageReport", "analyze_split_leakage", "read_manifest_records"]
...
def _overlap_report(
    ...
    if subj_overlap:
        return SplitLeakageReport(
            ...
            n_val=len(val),
            detail=(
                f"{len(subj_overlap)} subject id(s) appear in BOTH the train "
                f"and validation split — patient-level data leakage."
            ),
        )
    ...
    if file_overlap:
        return SplitLeakageReport(
            ...
            detail=(
                ...
                f"same on-disk volume is used for training and validation."
            ),
        )
    return SplitLeakageReport(
        status="clean",
        ...
        n_train=len(train),
        n_val=len(val),
        detail="train and validation splits are disjoint.",
    )
...
def analyze_split_leakage(config: Any) -> SplitLeakageReport:
    ...
    * directory / auto with no index files → disjoint by construction (skip).
    ...
    # is a runtime directory crawl (folder-disjoint by construction) → skip.
    if not index_path:
        return _skipped(
            "directory",
            "no index files declared; train/val resolved by directory crawl "
            "(folder-disjoint by construction).",
        )
