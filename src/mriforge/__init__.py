"""MRIForge: A multi-paradigm research framework for MRI reconstruction.
...
if TYPE_CHECKING:
    ...
    from mriforge.config.settings import (
        TrainingSettings as TrainingSettings,
    )
    from mriforge.config.settings import (
        settings_from_dict as settings_from_dict,
    )
    from mriforge.core.metrics.registry import register_metric as register_metric
    from mriforge.models.losses.registry import register_loss as register_loss
    from mriforge.models.registry import register_model as register_model
    from mriforge.pipelines.fit import (
        Trainer as Trainer,
    )
    from mriforge.pipelines.fit import (
        fit as fit,
...
# ``import mriforge.<anything>`` runs, including from notebooks, CLI
...
#
# ``import mriforge`` MUST stay cheap: eagerly importing these would drag in the
...
# subsequent accesses skip ``__getattr__`` entirely. ``mriforge.api`` is the
# eager facade for users who prefer one import; both resolve to the SAME
# canonical objects.
# ---------------------------------------------------------------------------
_LAZY_EXPORTS: dict[str, tuple[str, str]] = {
    "TrainingSettings": ("mriforge.config.settings", "TrainingSettings"),
    "settings_from_dict": ("mriforge.config.settings", "settings_from_dict"),
    "register_model": ("mriforge.models.registry", "register_model"),
    "register_loss": ("mriforge.models.losses.registry", "register_loss"),
    "register_metric": ("mriforge.core.metrics.registry", "register_metric"),
    "make_model": ("mriforge.pipelines.make", "make_model"),
    "make_optimizer": ("mriforge.pipelines.make", "make_optimizer"),
    "make_dataset": ("mriforge.pipelines.make", "make_dataset"),
    "make_dataloader": ("mriforge.pipelines.make", "make_dataloader"),
    "fit": ("mriforge.pipelines.fit", "fit"),
    "Trainer": ("mriforge.pipelines.fit", "Trainer"),
}
...
def __getattr__(name: str) -> Any:
    ...
    if target is None:
        raise AttributeError(f"module 'mriforge' has no attribute {name!r}")
...
def _emit_clinical_disclaimer() -> bool:
    """Warn once that MRIForge is research-only — main process only.

    Returns ``True`` if the warning was emitted, ``False`` if suppressed.
    Three gates silence it:

    * ``MRIFORGE_SUPPRESS_CLINICAL_WARNING`` is set (explicit opt-out).
    ...
    * We are a non-zero rank of a distributed launch: N interpreters means N
      ...
      the environment rather than through the ``mriforge.core.env`` SSOT that
      declares them. That began as a cost decision: importing the SSOT executed
      an eager ``mriforge.core.__init__`` that pulled torch, measured at 2.8 s
      and 4205 modules, paid by every ``import mriforge``. #1130 made that
    ...
    """
    if os.environ.get("MRIFORGE_SUPPRESS_CLINICAL_WARNING"):
        return False
    ...
    if "WORLD_SIZE" in os.environ and (os.environ.get("RANK", "0") != "0"):
        return False
    warnings.warn(
        "MRIForge is research software and is NOT FOR CLINICAL USE. "
        "It is not a medical device and has not been evaluated by any regulatory "
        "authority. See DISCLAIMER.md. "
        "Set MRIFORGE_SUPPRESS_CLINICAL_WARNING=1 to silence this message in "
        "batch jobs.",
