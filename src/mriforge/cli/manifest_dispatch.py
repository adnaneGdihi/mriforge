...
run the Tier-0/1 audit pre-flight, then ``mriforge train``.
...
--------------------
This module imports only stdlib + PyYAML, and ``mriforge.cli`` package init is
torch-free, so ``python -m mriforge.cli.manifest_dispatch`` runs on a torch-less
...
``python -m mriforge.cli`` AFTER the venv is activated, so the heavy imports
...
def _dispatch(
    *,
    ...
    print(f"[TRAIN] mriforge.cli {' '.join(train_args)}")
