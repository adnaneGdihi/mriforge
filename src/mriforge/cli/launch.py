"""Unified launcher — ``mriforge launch`` and the in-process ``LocalBackend``.
...
pipeline-agnostic :class:`MRIForgeInvocation` from ``--pipeline`` + ``--config``,
resolves a :class:`ResourceSpec`, and runs it. The existing dedicated commands
(``mriforge train``, ``sbatch …``, ``mriforge campaign submit``) all keep working
— ``launch`` is additive.
...
:mod:`mriforge.infrastructure.execution`.
"""
...
#: ``--config`` FLAG (``MRIForgeInvocation.to_cli_args`` → ``[verb, "--config",
...
#:   * ``audit`` — takes the config POSITIONALLY (``mriforge audit X.yaml``);
...
#: via the ``--`` passthrough (``MRIForgeInvocation.extra_args``).
PIPELINE_VERBS = (
    "train",
    "sanity_check",
    "infer",
...
class LocalBackend:
    ...
    Reuses the full ``mriforge`` command dispatch in-process, so every verb
    ...
    def run(
        ...
        *,
        ...
        if dry_run:
            return RunHandle(self.name, command=["mriforge", *argv])
        ...
        return RunHandle(self.name, returncode=returncode, command=["mriforge", *argv])
...
def launch(args: argparse.Namespace) -> int:
    ...
    # Hand the resolved backend + resources to the child run via MRIFORGE_LAUNCH_*
...
def _launch_campaign(args: argparse.Namespace) -> int:
    ...
    if args.where == "local":
        raise NotImplementedError(
            ...
            "slurm/docker/apptainer, or run a single config with `mriforge train`."
        )
    from mriforge.cli.app import main
