#!/usr/bin/env python
"""Unified CLI Entry Point for MRIForge System.

This provides a single, platform-independent entry point for all operations.

Usage:
    python -m mriforge.cli train --config experiments/active/dummy_gan.yaml
    python -m mriforge.cli predict --config run.yaml --model checkpoints/best.pt --input data/test/
...
def _dispatch_training(args: argparse.Namespace, *, is_sanity_check: bool) -> int:
    ...
    list and re-invoked ``mriforge.main:main()``, which re-parsed everything
    ...
    ``args``. Importing ``mriforge.main`` still runs its load-bearing
    ...
    if getattr(args, "debug", False):
        logging.getLogger("mriforge").setLevel(logging.DEBUG)
    ...
    from mriforge.main import sanity_check_command, train_command
...
def _parse_vary_specs(vary: list[str] | None) -> dict[str, dict[str, object]]:
    ...
    """
    from mriforge.config.overrides import _parse_value  # canonical value parser (SSOT)
...
def ablation(args: argparse.Namespace) -> int:
    ...
    """
    from mriforge.pipelines.ablation import run_ablation_study, train_and_score
...
def infer_dataset(args: argparse.Namespace) -> int:
    ...
    from mriforge.main import infer_dataset_command
...
def experiment(args: argparse.Namespace) -> int:
    ...
    args.seed = getattr(args, "seed", None)
    args.resume = getattr(args, "resume", None)
    from mriforge.main import experiment_command
...
def train_distributed(args: argparse.Namespace) -> int:
    """DDP training entry point (launch via ``torchrun ... -m mriforge.cli``)."""
    ...
    import mriforge.main  # noqa: F401  (side-effect import)
...
def predict(args: argparse.Namespace) -> int:
    ...
    ``DeprecationWarning`` promoted to an error under ``mriforge.*`` tests, and
    ...
    The training YAML is the Single Source of Truth for architecture +
    preprocessing, so ``--config`` is required at run time. It is an OPTIONAL
    argparse flag (so the config-less verb contract still parses) but, when
    absent, we fail loud with migration guidance rather than silently falling
    back to the removed deprecated path (pitfall #9).
    """
    config = getattr(args, "config", None)
    if config is None:
        logger.error(
            "predict now requires --config: the config-less inference path "
            "(InferencePipeline) is deprecated and no longer wired. Re-run with "
            "`--config <training.yaml>` (the SSOT that produced the checkpoint), "
            "e.g. `mriforge predict --config run.yaml --model best.pt --input data/`, "
            "or use the equivalent `mriforge infer --config ... --checkpoint ...`."
        )
        return 2

    # ``import mriforge.main`` also applies the pre-``import torch`` environment
    # setup (cache root, thread isolation, CUDA alloc) that every other verb in
    # this module relies on.
    import torch

    from mriforge.main import begin_inference_run
    ...
    _, device = begin_inference_run(config, getattr(args, "device", None), pipeline="predict")
...
def benchmark(args: argparse.Namespace) -> int:
    ...
    if args.suite in ["standard", "all"]:
        ...
        from mriforge.core.metrics.evaluation_metrics import NMSE, PSNR, SSIM
...
def export(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        from mriforge.shared.utils.data_utils import get_sample_batch
...
def list_features(args: argparse.Namespace) -> int:
    ...
    # ``mriforge`` distribution, so a bare ``from tools...`` only resolves when
...
def _gate_probe_acceleration(
    ...
    The ``run.device`` leg goes through :func:`mriforge.main._declared_device`
...
def _audit_one(args: argparse.Namespace, config_path: str) -> int:
    ...
    _witness_subject = WitnessSubject.for_audit(config_path=config_path, settings=config)
    _verdicts = run_witnesses(_witness_subject, tiers=frozenset({Tier.T0, Tier.T1}))
    report = HealthCheckReport(results=[verdict_to_health_result(v) for v in _verdicts])
    ...
    if args.probe and probe_decision is not None:
        ...
        # mriforge.infrastructure.validation.operator_id_probe.
        if getattr(getattr(config, "training", None), "equivariant_imaging", None) is not None:
            ...
            from mriforge.infrastructure.validation.operator_id_probe import (
...
def _audit_bulk(args: argparse.Namespace) -> int:
    ...
    for path in yamls:
        ...
        try:
            report = validate_config_health(cfg)
        except Exception as exc:
            n_schema_err += 1
            n_err += 1
            console.print(
                f"[bold red]❌[/] {rel:<70}  [red]validate_config_health crashed:[/] {str(exc)[:80]}"
            )
            continue
        ...
        n_e = sum(1 for x in report.results if getattr(x, "severity", "") == "error")
        n_w = sum(1 for x in report.results if getattr(x, "severity", "") == "warning")
        outcome: str
        icon_markup: str
        outcome_style: str
        if n_e:
    ...
    if rec_code_counts:
        from mriforge.infrastructure.validation.recommendations import DIAGNOSTIC_CODES
...
def hpo_cmd(args: argparse.Namespace) -> int:
    ...
    Each trial spawns a separate ``python -m mriforge.cli train`` subprocess
    ...
    if len(preset_names) == 1:
        ...
        from mriforge.pipelines.hpo_search_spaces import load_presets
...
def report(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "recursive", False):
        from mriforge.infrastructure.reporting import generate_reports
...
def _load_clean_volumes_from_pt(
    ...
    Delegates to :class:`mriforge.data.datasets.meta_eval_clean_refs.MetaEvalCleanRefsDataset`
...
def _load_clean_volumes_from_h5(
    ...
    from mriforge.data.datasets.m4raw_repetition_groups import (
...
def _resolve_device(spec: str | None, *, pipeline: str = "evaluate") -> "torch.device":
    ...
    Thin shell over the SSOT policy in :mod:`mriforge.core.compute_device`.
...
def meta_evaluate(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "tables", False):
        from mriforge.core.metrics.meta_evaluation import write_tables
...
def build_parser() -> argparse.ArgumentParser:
    """Construct the full ``mriforge`` argument parser (every subcommand).
    ...
    injects (``mriforge launch <cfg> --pipeline <verb>`` would otherwise fail at
    ...
    parser = argparse.ArgumentParser(
        prog="mriforge",
        description="MRIForge: Medical Image Super-Resolution and Reconstruction",
    )
    from mriforge.cli.diagnostics import _version as _mriforge_version

    parser.add_argument(
        "--version",
        action="version",
        version=f"mriforge {_mriforge_version()}",
    )
    parser.add_argument(
        "-v",
        "--verbose",
        action="store_true",
        help="Print full tracebacks on error (or set MRIFORGE_DEBUG=1).",
    )
    ...
    from mriforge.cli.diagnostics import run_doctor
    ...
    dist_parser = subparsers.add_parser(
        "train-distributed",
        help="DDP training (launch via: torchrun --nproc_per_node=N -m mriforge.cli train-distributed ...)",
    )
    ...
    predict_parser = subparsers.add_parser(
        "predict", help="Run inference via the SSOT pipeline (requires --config)"
    )
    ...
    try:
        from mriforge.cli.audit_plan_novel_cli import attach_subparsers as _attach_novel
    ...
    try:
        from mriforge.cli.design_mrf_sequence_cli import attach_subparsers as _attach_mrf
    ...
    try:
        from mriforge.cli.regulatory import add_arguments as _attach_regulatory
    ...
    from mriforge.cli.launch import (
        FANOUT_CHOICES,
        PIPELINE_VERBS,
        WHERE_CHOICES,
        launch,
    ...
    # ``mriforge profile`` — run a pipeline verb under Scalene. Attached
    ...
    from mriforge.cli.profile_cli import attach_subparsers as _attach_profile
...
def _emit_startup_notice(command: str | None) -> bool:
    ...
    ``MRIFORGE_QUIET`` / ``MRIFORGE_SUPPRESS_CLINICAL_WARNING`` are set (the same
    ...
    if command not in _HEAVY_STARTUP_COMMANDS:
        return False
    if os.environ.get("MRIFORGE_QUIET") or os.environ.get("MRIFORGE_SUPPRESS_CLINICAL_WARNING"):
        return False
    from mriforge.core import env as _env

    if _env.is_secondary_rank():
        return False
    print(
        f"⏳ mriforge {command}: importing PyTorch + model registry "
...
def main(argv: list[str] | None = None) -> int:
    ...
    Args:
        argv: Optional argument list (defaults to ``sys.argv``). Passed so the
            in-process ``LocalBackend`` (``mriforge launch --where local``) can
    ...
    from mriforge.infrastructure.logging.rank_console import quiet_secondary_ranks
    ...
    # exception → concise error + exit 1 (full traceback only with -v / MRIFORGE_DEBUG).
    verbose = bool(getattr(args, "verbose", False)) or bool(os.environ.get("MRIFORGE_DEBUG"))
    ...
    try:
        ...
        if verbose:
            ...
            logger.error("Re-run with -v / --verbose (or MRIFORGE_DEBUG=1) for a full traceback.")
        return 1
