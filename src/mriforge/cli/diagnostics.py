"""``mriforge doctor`` — environment & wiring diagnostics for cluster use.
...
determinism flags, the resolved cache/data roots, the relevant ``MRIFORGE_*``
...
def _version() -> str:
    try:
        return importlib.metadata.version("mriforge")
...
def _torch_info() -> dict[str, Any]:
    ...
    :func:`mriforge.infrastructure.logging.provenance.torch_runtime` (the SSOT
    ...
    """
    from mriforge.infrastructure.logging.provenance import torch_runtime
...
def collect_diagnostics() -> dict[str, Any]:
    ...
    from mriforge.infrastructure.config.env_resolver import resolve_cache_root
    ...
    data_root = os.environ.get("MRIFORGE_DATA_ROOT", "databases/")
    return {
        "version": _version(),
...
def run_doctor(args: argparse.Namespace) -> int:
    ...
    command doubles as a cluster pre-flight gate (``mriforge doctor --require-cuda``).
    """
