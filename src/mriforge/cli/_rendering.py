...
- ``python -m mriforge.cli audit foo.yaml`` in an interactive terminal renders
  green ✅ / red ❌ / yellow ⚠ with cyan check-name tags.
- ``python -m mriforge.cli audit foo.yaml > out.txt`` writes plain text
  identical to the pre-round-11 ``print(...)`` output. CI / grep /
  downstream parsers are unaffected.
- ``python -m mriforge.cli audit foo.yaml --json`` is unaffected — the JSON
