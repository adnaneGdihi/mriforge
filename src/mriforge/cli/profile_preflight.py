...
``mriforge profile --target train`` pays for the entire training environment —
...
* the process-group requirement is asked of the strategy plugins themselves via
  :attr:`~mriforge.infrastructure.distributed.strategy_registry.ParallelStrategyPlugin.requires_process_group`,
...
* the validation-gate arithmetic is :func:`mriforge.pipelines.train.validation_can_fire`
...
def extract_overrides(extra: list[str] | None) -> list[str]:
    ...
    :func:`mriforge.config.overrides.apply_overrides`, never interpreted here.
    """
...
def check_parallel_strategy(*, settings: Any, target: str) -> None:
    ...
    raise ValueError(
        ...
        f"initialized torch.distributed process group, and `mriforge profile` "
...
def check_validation_interval(*, settings: Any, target: str) -> None:
    ...
    * ``target`` is ``sanity_check`` — see :data:`INTERVAL_TARGETS`.
    ...
    from mriforge.pipelines.train import validation_can_fire
