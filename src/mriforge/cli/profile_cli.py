"""``mriforge profile`` — run a pipeline verb under Scalene.
...
the argv they produce, live in :mod:`mriforge.cli.profile_command`; the artifact
layout lives in :mod:`mriforge.cli.profile_paths`.
...
:class:`~mriforge.cli.profile_paths.ProfilePaths`). Checks that refuse a run
before Scalene starts live in :mod:`mriforge.cli.profile_preflight`.
"""
...
from mriforge.cli.profile_diagnostics import explain_failure
...
from mriforge.cli.profile_preflight import run_preflight
...
def _split_phases(paths: ProfilePaths) -> dict[str, Any]:
    ...
    """
    from mriforge.cli.profile_phases import write_phase_reports
...
def profile(args: argparse.Namespace) -> int:
    ...
    from mriforge.infrastructure.logging.provenance import git_provenance, make_run_id
