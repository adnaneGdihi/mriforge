...
:mod:`mriforge.pipelines.train` (a same-layer import); a follow-up (PR-2) inlines
...
from mriforge.config.overrides import applied_override_paths
from mriforge.core.cascading_validation import aggregate_cascade_rows
...
def resolve_ema_warmup_gate(ema: Any, ema_cfg: Any) -> int:
    ...
    * adaptive — a soft decay ramp ``initial_decay -> final_decay`` applied
      inside :class:`~mriforge.infrastructure.optimization.ema.ModelEma`, where
...
def _execute_training_loop(
    ...
    # nothing under src/mriforge ever called it. This is the driver.
        ...
        if iteration % log_interval == 0 or is_first_iteration or is_last_iteration:
            ...
            if pipeline.schedulers:
                for sched_name, sched in pipeline.schedulers.items():
                    try:
                        current_lr = (
                            sched.get_last_lr()[0] if hasattr(sched, "get_last_lr") else None
                        )
