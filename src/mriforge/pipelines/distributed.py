...
Usage::
    ...
    torchrun --nproc_per_node=4 -m mriforge.pipelines.distributed --config <arm>.yaml
"""
...
def setup_distributed(backend: str = "nccl") -> tuple[int, int]:
    ...
    if rank == 0:
        ...
        # again here covers ``python -m mriforge.pipelines.distributed``, which
...
def run_distributed_training(
    ...
    # ``mriforge train-distributed`` path, but ``python -m
    # mriforge.pipelines.distributed`` bypasses it -- and everything between here
    ...
    try:
        ...
        # ``mriforge.main`` — a pipelines→entry-layer leftward import (CLAUDE.md
        ...
        from mriforge.infrastructure.distributed.backend import (
...
def main() -> None:
    """CLI entry point for ``python -m mriforge.pipelines.distributed``."""
