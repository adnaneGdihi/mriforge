...
Legacy manual service instantiation has been replaced with `mriforge.bootstrap` builder.
...
from mriforge.infrastructure.services.tensorboard_writer import TensorBoardWriter
...
from mriforge.core.metrics.output_sanity import measure_output_sanity
...
from mriforge.infrastructure.builders.directors import CheckpointDirector
...
from mriforge.infrastructure.services.metric_keys import (  # noqa: F401
    early_stop_monitor_candidates,
)
from mriforge.infrastructure.training.builders.director import (
...
from mriforge.infrastructure.validation.resolved_config_artifact import (  # noqa: E402
    write_ledger_failure_sentinel,
    write_resolved_config,
)
from mriforge.models.analysis.extensive_gradient_logger import ExtensiveGradientLogger
from mriforge.models.stability.manager import StabilityManager
from mriforge.pipelines.parallel import is_rank_zero, resolve_data_rank
from mriforge.shared.utils.seed_control import set_global_seed
...
def run_training_pipeline(
    ...
    try:
        from mriforge.infrastructure.logging import banner as _banner
    ...
    from mriforge.infrastructure.logging.provenance import log_startup_summary
    ...
    if config.logging.identity.run:
        ...
        run_name = config.metadata.get("name", "unnamed_run")
    ...
    try:
        from mriforge.infrastructure.logging.provenance import collect_run_provenance
    ...
    try:
        from mriforge.infrastructure.logging.provenance import rank_device_inventory
    ...
    try:
        ...
        try:
            from mriforge.infrastructure.logging.provenance import log_provenance
    ...
    *,
    ...
    try:
        ...
        summary = {
            ...
            or (config.metadata.get("name") if getattr(config, "metadata", None) else None),
            "seed": seed,
            ...
            # Backend + resolved resources when started via ``mriforge launch``
            # (pitfall #15c); None for a plain ``mriforge train``.
...
def _write_hparams(tb_writer: TensorBoardWriter, config: Any) -> None:
    ...
    try:
        ...
        # ``metadata`` is ``dict[str, Any] | None`` on TrainingSettings, so this
        # has to be a key lookup: ``getattr(a_dict, "baseline", None)`` is always
        # None, and the baseline axis never reached the dashboard.
        metadata = config.metadata or {}
        baseline = (
            metadata.get("baseline")
            if isinstance(metadata, dict)
            else getattr(metadata, "baseline", None)
        )
        if baseline:
...
from mriforge.infrastructure.reporting.run_hook import (  # noqa: E402,F401
    UNCONFIGURED_REPORT_TABLES as _UNCONFIGURED_REPORT_TABLES,
)
from mriforge.infrastructure.reporting.run_hook import (  # noqa: E402
...
def _column_higher_is_better(key: str) -> bool | None:
    ...
    Delegates to :func:`~mriforge.core.metrics.metric_directions.resolve_direction`
...
def _all_reduce_val_metrics(val_accum, val_weight, device):
    ...
    (:func:`mriforge.pipelines.parallel._apply_distributed_samplers`) which
    *shards and pads* the val set, so each rank's ``val_accum`` / ``val_weight``
    ...
    initialised — i.e. on the default single-process ``mriforge train`` path,
