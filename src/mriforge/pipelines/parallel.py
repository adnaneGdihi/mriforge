...
``mriforge.infrastructure.distributed`` (see the note below). What remains is the
...
# ZERO callers in ``src/``.
#
# The implementation now lives in ``mriforge.core.module_utils``, the rightmost
...
# ``mriforge.infrastructure.distributed.strategy_registry``, dispatched from
...
#: ``unwrap_model`` is re-exported from ``mriforge.core.module_utils``; naming it
