...
Trial isolation
---------------
``subprocess_training_objective`` spawns ``python -m mriforge.cli train`` as a
...
def subprocess_training_objective(
    ...
    3. Spawns ``python -m mriforge.cli train --config ...`` as a subprocess.
    ...
    def _objective(trial: "optuna.trial.Trial") -> float:  # noqa: F821
        ...
        cmd = [sys.executable, "-m", "mriforge.cli", "train", "--config", str(yaml_path)]
