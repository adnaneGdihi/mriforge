...
The imperative peer of the config-driven ``mriforge train``: a user brings their
...
:func:`mriforge.pipelines.train.run_training_pipeline` with that environment, so
...
def fit(
    ...
    *,
    ...
    if paradigm == "gan":
        disc = discriminator or _build_discriminator_from_config(settings, torch_device)
        if disc is None:
            raise ValueError(
        ...
        # same YAML gave D its own LR under `mriforge train`.
        optimizers["opt_d"] = (
            opt_d
    ...
    return run_training_pipeline(settings, device=str(torch_device), env=env, strategy=strategy)
...
class Trainer:
    ...
    ``mriforge.infrastructure.training.step_executor.StepExecutor``
    ...
    def predict(
        ...
        *,
        ...
        Delegates to :func:`mriforge.pipelines.infer.run_inference_pipeline`,
