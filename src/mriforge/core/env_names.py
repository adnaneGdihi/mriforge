"""Registry of every environment-variable NAME MRIForge reads.

Names only -- this module registers, it does not read. The parsed accessors live
in :mod:`mriforge.core.env`, which re-exports every constant below, so
``from mriforge.core import env`` remains the one import every consumer needs.
...
``.env.example`` coverage. That is exactly how ``MRIFORGE_GPU_MEMORY_FRACTION``
...
One family is deliberately absent: ``MRIFORGE_LAUNCH_*`` is composed at runtime
from a prefix in ``infrastructure.execution.backends`` and written by
``mriforge launch`` for the child process to read back as provenance. It is an
...
MRIFORGE_DATA_ROOT = "MRIFORGE_DATA_ROOT"
...
MRIFORGE_ROOT = "MRIFORGE_ROOT"
MRIFORGE_CLUSTER_ROOT = "MRIFORGE_CLUSTER_ROOT"
MRIFORGE_CACHE_ROOT = "MRIFORGE_CACHE_ROOT"
MRIFORGE_LEGACY_ABS_PREFIXES = "MRIFORGE_LEGACY_ABS_PREFIXES"
MRIFORGE_LEGACY_CLUSTER_PREFIX = "MRIFORGE_LEGACY_CLUSTER_PREFIX"
...
MRIFORGE_SUPPRESS_CLINICAL_WARNING = "MRIFORGE_SUPPRESS_CLINICAL_WARNING"
FORCE_CPU = "FORCE_CPU"
MRIFORGE_DEVICE = "MRIFORGE_DEVICE"
MRIFORGE_NO_GPU_PROBE = "MRIFORGE_NO_GPU_PROBE"
...
MRIFORGE_GPU_MEMORY_FRACTION = "MRIFORGE_GPU_MEMORY_FRACTION"
...
MRIFORGE_DIMENSION_CONTRACT = "MRIFORGE_DIMENSION_CONTRACT"
...
MRIFORGE_ALLOW_MAMBA_FALLBACK = "MRIFORGE_ALLOW_MAMBA_FALLBACK"
...
MRIFORGE_PLUGINS = "MRIFORGE_PLUGINS"
...
MRIFORGE_LEDGER_STRICT = "MRIFORGE_LEDGER_STRICT"
...
#: ``MRIFORGE_DEBUG=0`` ENABLES it. Unset the variable to turn it off.
MRIFORGE_DEBUG = "MRIFORGE_DEBUG"
#: Suppress the CLI startup notice. Same bare-truthiness read as
#: ``MRIFORGE_DEBUG`` above: ``MRIFORGE_QUIET=0`` is quiet. Unset to turn off.
MRIFORGE_QUIET = "MRIFORGE_QUIET"
...
#: ``mriforge train`` and to nothing else, and the multi-GPU path
...
# image names as module-level defaults (``mriforge:v6.1``, ``mriforge.sif``).
...
MRIFORGE_SLURM_MAIL_USER = "MRIFORGE_SLURM_MAIL_USER"
#: Apptainer/Singularity ``.sif`` image for the container execution backend.
MRIFORGE_APPTAINER_SIF = "MRIFORGE_APPTAINER_SIF"
