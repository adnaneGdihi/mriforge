...
:func:`~mriforge.core.metrics.registry.register_metric`, a self-declared
``higher_is_better`` property, and a ``__call__`` that reads its
measurement/geometry context through
:func:`~mriforge.core.metrics.context.resolve_context`. A missing or
...
def _as_bchw_magnitude(prediction: torch.Tensor) -> torch.Tensor:
    ...
    :func:`mriforge.core.metrics.hallucination_metrics._to_magnitude` for the
