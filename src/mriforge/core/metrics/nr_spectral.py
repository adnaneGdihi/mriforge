...
:class:`~mriforge.core.metrics.context.MetricContext`.
...
* All four return a Python ``float`` and *never* raise ``ValueError``
  ...
  ``float('nan')``.
* ``ssel`` depends on ``mriforge.infrastructure.physics.prolate`` (a sibling
...
from mriforge.infrastructure.physics.hermitian import _cyclic_mirror
...
def _phase_lowpass_unitvector(
    ...
    """
    from mriforge.models.losses.high_frequency_residual_loss import _gaussian_lowpass
...
class SlepianSubspaceEnergyLeakage:
    ...
    def __call__(
        ...
        **kwargs: object,
        ...
        try:
            from mriforge.infrastructure.physics.prolate import slepian_concentration
