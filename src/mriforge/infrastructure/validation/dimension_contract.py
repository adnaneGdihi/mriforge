...
def resolve_contract_mode() -> str:
    """Resolve the runtime guard mode from ``MRIFORGE_DIMENSION_CONTRACT``.
    ...
    * ``enforce`` — raise :class:`DimensionContractError` at iteration 1.
    ...
    """
    raw = os.environ.get("MRIFORGE_DIMENSION_CONTRACT", "observe").strip().lower()
    if raw not in _VALID_MODES:
        raise ValueError(
            f"MRIFORGE_DIMENSION_CONTRACT={raw!r} is invalid; expected one of {_VALID_MODES}."
        )
    return raw
