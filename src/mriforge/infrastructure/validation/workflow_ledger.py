...
The frozen :class:`~mriforge.domain.workflows.profiles.WorkflowProfile` facts
...
* **A broken ``mriforge`` strategy import raises.** Swallowing it would silently
...
def signal_model_registered(name: str | None) -> bool:
    ...
    :mod:`mriforge.infrastructure.physics.signal_models.registry`).
...
def metrics_tagged(regime: Regime) -> bool:
    ...
    Importing :mod:`mriforge.core.metrics` triggers the package's
    ``pkgutil.walk_packages`` auto-discovery, so every ``@register_metric``
    decorator has fired before we read the tag table.
    """
    import mriforge.core.metrics  # noqa: F401  (side-effect: populate registry)
...
def losses_tagged(regime: Regime) -> bool:
    ...
    import mriforge.models.losses  # noqa: F401  (side-effect: populate registry)
...
def _iter_strategy_classes() -> Iterator[type[Any]]:
    ...
    for dotted in TrainingStrategyFactory.STRATEGY_CLASS_PATHS.values():
        ...
        try:
            ...
            if (exc.name or "").split(".")[0] == "mriforge":
                raise ImportError(
                    f"workflow_ledger: {dotted} names a mriforge module that "
