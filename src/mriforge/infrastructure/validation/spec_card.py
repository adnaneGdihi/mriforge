...
def _derive_data_form(config: Any) -> dict[str, Any]:
    ...
    # First pass: infer from dataset_type. The kspace family includes
    # any dataset that delivers raw multi-coil k-space — m4raw, fastmri,
    # explicit `kspace`, plus the `fastmri_kspace` short alias.
    # One home for the family (mriforge.data.signal_domain): the data layer needs
    # the same fact and cannot import infrastructure/ (#5).
    from mriforge.data.signal_domain import is_kspace_dataset_type

    domain_inferred = "image"
    if is_kspace_dataset_type(dataset_type):
        domain_inferred = "kspace"
    if dataset_type and dataset_type == "pde_synthetic":
        domain_inferred = "pde_grid"
    # Second pass: coil_processing_mode can transform k-space into image
    # before the model sees it. `rss_image` and `magnitude` both produce
    # 1-channel real magnitude, regardless of source dataset_type.
