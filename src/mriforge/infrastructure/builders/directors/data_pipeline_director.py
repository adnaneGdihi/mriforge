...
if TYPE_CHECKING:
    ...
    from mriforge.config.settings import TrainingSettings  # noqa: F401  (docstring ref)
    ...
    from mriforge.data.builders.site_balancer import Balancer
...
def _build_slice_sampler(dataset) -> object | None:
    """A :class:`~mriforge.data.samplers.VolumeBlockedSliceSampler` for a slice-expanded
    dataset, else ``None``.
...
def apply_coil_processing(
    ...
    from mriforge.infrastructure.physics.coil_sensitivity import estimate_smaps
    from mriforge.infrastructure.physics.fft_ops import coil_combine, ifft2c
...
class DataPipelineDirector:
    ...
    def build_multi_domain_dataloaders(
        ...
        Returns:
            A :class:`Balancer` (from :py:mod:`mriforge.data.builders.site_balancer`)
        ...
        """
        from mriforge.data.builders.site_balancer import build_balancer
