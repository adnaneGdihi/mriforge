#!/usr/bin/env python3
"""Logging Service for MRIForge project.
...
from mriforge.infrastructure.logging.rank_console import rank_floor
...
class ColoredConsoleFormatter(std_logging.Formatter):
    ...
    def _abbreviate_name(self, name: str, max_len: int = 28) -> str:
        """Shorten logger names like 'mriforge.infrastructure.training.strategies.diffusion' → 's.i.t.s.diffusion'."""
        parts = name.split(".")
...
class LoggingService(ILoggingService):
    ...
    def setup(
        ...
        if log_dir and log_to_file:
            ...
            if not has_file_handler:
                try:
                    ...
                    # likely to be lost.
                    fallback_dir = tempfile.mkdtemp(prefix="mriforge_logs_")
        ...
        problematic_modules = [
            "mriforge.application.orchestration.pipeline_orchestrator",
            "mriforge.data.factories.dataloader_factory",
            "mriforge.data.loaders.async_data_loader",
        ]
