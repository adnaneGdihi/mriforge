...
_EXAMPLE_ROOT = '"/tmp/$USER/mriforge_cache"'
...
def _create_cache_dir(var: str, value: str, *, framework_value: str | None) -> None:
    ...
    moves it, since only ``MRIFORGE_CACHE_ROOT`` (whole block) and
    ...
    try:
        ...
        if framework_value is None:
            ...
            raise CacheDirectoryError(
                ...
                f"    export MRIFORGE_CACHE_ROOT={_EXAMPLE_ROOT}"
            ) from exc
        if value != framework_value:
            ...
            remedy = (
                ...
                "MRIFORGE_CACHE_ROOT is the control point: exporting any member "
...
def resolve_data_root() -> Path:
    ...
    Resolution order:
    1. MRIFORGE_DATA_ROOT environment variable (explicit user override)
    2. Default to ~/.cache/mriforge/data (local machine)
    ...
    Example:
        >>> # User can set: MRIFORGE_DATA_ROOT=/mnt/datasets/mriforge
        >>> # Or leave default: ~/.cache/mriforge/data
        >>> root = resolve_data_root()
        >>> print(root)  # /home/user/.cache/mriforge/data
    """
    env_path = os.environ.get("MRIFORGE_DATA_ROOT")

    if env_path:
        data_root = Path(env_path)
        logger.info(f"Data root from MRIFORGE_DATA_ROOT: {data_root}")
    else:
        # Fallback to local cache directory
        data_root = Path.home() / ".cache" / "mriforge" / "data"
...
def resolve_cache_root() -> Path:
    ...
    Resolution order:
    1. MRIFORGE_CACHE_ROOT environment variable (explicit user override)
    2. TMPDIR environment variable (cluster standard)
    3. Default to ~/.cache/mriforge/torch (local machine)
    ...
    Example:
        ...
        >>> # Local: uses ~/.cache/mriforge/torch
        >>> cache = resolve_cache_root()
    """
    cache_path = os.environ.get("MRIFORGE_CACHE_ROOT")

    if cache_path:
        cache_root = Path(cache_path)
        source = "MRIFORGE_CACHE_ROOT"
        logger.info(f"Cache root from MRIFORGE_CACHE_ROOT: {cache_root}")
    elif os.environ.get("TMPDIR"):
        # Use cluster TMPDIR if available
        cache_root = Path(os.environ.get("TMPDIR")) / "mriforge_cache"
        source = "TMPDIR"
        logger.info(f"Cache root from TMPDIR: {cache_root}")
    else:
        # Fallback to local cache
        cache_root = Path.home() / ".cache" / "mriforge" / "torch"
    ...
    # ``$TMPDIR/mriforge_cache/mriforge_cache``, so torch/CUDA caches landed under
    ...
    os.environ["MRIFORGE_CACHE_ROOT"] = str(cache_root)
    return cache_root


#: Cache variables pointed at ``cache_root``, and whether an existing value wins.
#:
#: ``MRIFORGE_CACHE_ROOT`` is the documented control point for the first five, so
...
def configure_cache_environment(cache_root: Path | None = None) -> dict[str, str]:
    ...
    ``mriforge/main.py`` and was therefore applied on the ``mriforge train`` path
    **only** -- ``torchrun -m mriforge.cli train-distributed`` (the documented
...
def resolve_device() -> str:
    ...
    Resolution order:
    1. MRIFORGE_DEVICE environment variable (explicit user override)
    2. Default to "auto" — resolved by the accelerated-run contract in
       :mod:`mriforge.core.compute_device`, which picks CUDA when present and
    ...
    Example:
        >>> # Override: MRIFORGE_DEVICE=cuda
        >>> # Or default: cpu (requires explicit override to use GPU)
        >>> device = resolve_device()
        >>> print(device)  # "cpu" or "cuda" depending on env
    """
    device = os.environ.get("MRIFORGE_DEVICE", "auto").lower()

    valid_devices = {"cuda", "cpu", "auto"}
    if device not in valid_devices:
        raise ValueError(
            f"Invalid device '{device}'. Must be one of: {valid_devices}. "
            f"Set via MRIFORGE_DEVICE environment variable."
        )

    logger.info(f"Device from MRIFORGE_DEVICE or default: {device}")
    return device
