...
3. Runs ``optimizer.optimize(objective_fn, model_type, n_trials)`` where
   ...
   ``mriforge.pipelines.hpo.subprocess_training_objective``.
...
``python -m mriforge.cli train --config <generated.yaml>`` so a trial crash
...
class HPOCoordinator:
    ...
    def execute_hpo(
        ...
        try:
            from mriforge.models.tuning.enhanced_hyperparameter_tuning import (
                EnhancedHPOConfig,
                EnhancedHPOptimizer,
        ...
        # allowed to import from ``mriforge.pipelines``). The infrastructure
        ...
        if objective_factory is None:
            raise ValueError(
                ...
                "(typically `mriforge.pipelines.hpo.subprocess_training_objective`). "
