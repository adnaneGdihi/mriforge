...
def resolve_model_in_channels(module: Any) -> tuple[int | None, str]:
    """Best-effort read of the backbone's declared input width.

    Preference order, most authoritative first:

    1. a module-level ``in_channels`` attribute -- what the architecture itself
       says it takes, and the only source that survives an unusual first layer;
    2. the first ``Conv{1,2,3}d`` in ``modules()`` order;
    3. the first ``Linear``'s ``in_features``.
