...
class CrossContrastKspaceDiffusionStrategy(DiffusionTrainingStrategy):
    ...
    def _compute_losses_impl(
        ...
        **kwargs: Any,
        ...
        k_src = batch.get("kspace_source") if isinstance(batch, dict) else None
        k_dst = batch.get("kspace_target") if isinstance(batch, dict) else None
        if k_src is None or k_dst is None:
            ...
            # denoiser. No dataset under src/mriforge/data/ emits either key, so
