    ...
    def validation_step(
        ...
        **kwargs: Any,
        ...
        with torch.no_grad():
            ...
            self._last_visual_pred = pred_mag.unsqueeze(1) if pred_mag.ndim == 3 else pred_mag
            self._last_visual_target = (
                target_mag.unsqueeze(1) if target_mag.ndim == 3 else target_mag
            )
        ...
        if computer is not None:
            ...
            try:
                result.update({k: float(v) for k, v in computer.compute(pm, tm).items()})
