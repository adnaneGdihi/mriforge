...
from mriforge.models.conditioning import AdaptiveConditioner, ConditioningContext
...
class ConcreteVirtualFiducialStrategy(BaseTrainingStrategy):
    ...
    def validation_step(
        ...
        **kwargs: Any,
        ...
        with torch.no_grad():
            ...
            # the target IFFT above. Without this, the cached "fake" visual and
            # val_psnr are computed on raw |k-space| (4-corner-blob / ring artifact,
            # F1 / TODO/audit/smoke_audit_20260526.md). infer_output_domain is the
            # authoritative SSOT (raises on inconsistency; no silent fallback).
            ...
            # Cache image-domain magnitude visuals for the pipeline's
            # image logging.  Without this, train.py falls back to a raw
    ...
    def _score_field(self, real_reference: bool = False) -> dict[str, float]:
        ...
        try:
            ...
            from mriforge.core.metrics.quantitative.qmri_agreement import (
