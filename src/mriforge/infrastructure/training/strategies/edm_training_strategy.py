...
(``mriforge.models.diffusion.edm_schedule``) into the diffusion training
...
class EDMTrainingStrategy(DiffusionTrainingStrategy):
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        from mriforge.models.diffusion.edm_schedule import EDMNoiseSchedule
    ...
    def _compute_losses_impl(
        ...
        **kwargs: Any,
        ...
        clean = batch.get("target") if isinstance(batch, dict) else batch
        if clean is None:
            return {"loss_total": torch.tensor(0.0, device=self.device)}
