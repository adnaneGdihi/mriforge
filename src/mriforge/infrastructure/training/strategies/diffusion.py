...
from mriforge.data.batch_types import align_scale_to_batch, read_batch_field
...
from mriforge.models.losses.computers import UnifiedDiffusionLossComputer
from mriforge.models.losses.computers.unified_diffusion_reconstruction import (
    _call_safe_loss,
)
from mriforge.models.losses.kspace_physics_losses import FrequencyWeightedL1Loss
...
class DiffusionTrainingStrategy(BaseTrainingStrategy, DiffusionStrategyMixin):
        ...
        **kwargs: Any,
        ...
        with torch.no_grad():
            ...
            self._stamp_accel_mean(
                all_metrics,
                _CASCADING_LEVELS,
                self._configured_validation_metrics(),
    ...
    def _maybe_hallucination_metrics(
        ...
        if step % interval != 0:
            return {}
        try:
            from mriforge.core.metrics.hallucination_test import run_hallucination_test
        ...
        *,
        ...
        try:
            ...
            if is_cold_diffusion:
                ...
                if _multistep and hasattr(gen, "sample"):
    ...
    def _compute_validation_metrics(
        ...
        try:
            ...
            _needs_ifft_preds, _ = needs_ifft_for_visualization(self.config)
        except Exception:  # pragma: no cover — defensive only
    ...
    def _log_validation_images_to_tensorboard(
        ...
        try:
            ...
            if logging_config and logging_config.images.log_input:
                input_img = kspace_to_image(inputs)
                input_mag = _ensure_4d(to_magnitude(input_img))
                input_mag = _reduce_to_single_channel(input_mag)
