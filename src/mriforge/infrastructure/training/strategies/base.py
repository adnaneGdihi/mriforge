...
if TYPE_CHECKING:
    ...
    from mriforge.infrastructure.training.debug_snapshot import ChannelSegment
...
from mriforge.infrastructure.training.metrics.metrics_reporter import MetricsReporter
...
from mriforge.infrastructure.training.strategies.loss_folding import scheduled_overrides
...
from mriforge.infrastructure.training.strategies.mixins.utils import (  # noqa: F401  # noqa: F401
    _CALLABLE_KWARG_CACHE,
    _callable_accepts_kwarg,
    _get_config_value,
    pick_present,
)
from mriforge.infrastructure.training.strategies.mixins.validation import ValidationMixin
...
class BaseTrainingStrategy(
    ...
    #: fields). See :mod:`mriforge.models.capabilities`.
    #:
    ...
    def __init__(
        ...
        **kwargs: object,
        ...
        from mriforge.infrastructure.builders.leaf.adapter_builders import (
            AdapterChainBuilder,
        )
    ...
    def apply_adapters(self, hook: str, x: Any) -> Any:
        ...
        just runs it.
        """
        from mriforge.infrastructure.builders.leaf.adapter_builders import apply_chain
    ...
    def _ensure_image_domain_target(self, target: torch.Tensor) -> torch.Tensor:
        ...
        **Domain decision is delegated to the SSOT**
        ...
        Only ``ifft2c`` from :mod:`mriforge.infrastructure.physics.fft_ops` is used
    ...
    def save_debug_snapshot(
        ...
        *,
        ...
        See :mod:`mriforge.infrastructure.training.debug_snapshot` for the
        ...
        :func:`~mriforge.infrastructure.training.debug_snapshot._resolve_deferred_extra`.
    ...
    def _verify_model_input_snapshot(
        ...
        * the WIDTH half warns and stamps. Resolving a backbone's input width is
          a heuristic (see
          :func:`~mriforge.infrastructure.training.model_input_contract.resolve_model_in_channels`),
    ...
    def _capture_model_output(
        ...
        * **Total-loss key normalization.** Every bypass site reads
        ...
        ``MRIFORGE_DIMENSION_CONTRACT=enforce``; installing it from here would
    ...
    def _ensure_input_contract_guard(self) -> None:
        ...
        ``MRIFORGE_DIMENSION_CONTRACT`` knob (``observe`` default → log-only;
        ...
        *,
        ...
        Args:
            ...
            channel_segments: For a key whose channel axis superposes domains,
                ...
                decomposition. See
                :func:`~mriforge.infrastructure.training.debug_snapshot.save_debug_snapshot`.
    ...
    def _snapshot_declared_model_input(self, *, step: int) -> None:
        ...
        * A missing declaration is an ARTIFACT defect, so it is enforced on the
          ...
          :func:`~mriforge.infrastructure.training.debug_snapshot.snapshots_are_enabled`.
    ...
    def _get_loss_weight(self, loss_name: str, epoch: int = 0, **kwargs: Any) -> float:
        ...
        :mod:`mriforge.models.losses.weights`, and an undeclared loss RAISES.
        """
