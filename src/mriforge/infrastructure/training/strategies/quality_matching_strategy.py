...
from mriforge.infrastructure.physics.chain_fitter import FitResult, fit_chain
...
def _cohort_spacing(path: str) -> tuple[float, float, float]:
    ...
    from mriforge.infrastructure.physics.quality_descriptors import read_spacing_mm
...
def measure_cohort_acquisition(
    ...
    from mriforge.infrastructure.physics.acquisition_params import AcquisitionParams
...
def _measure_volume(path: str, attributes: Sequence[str]) -> dict[str, float]:
    ...
    stays inside ``src/mriforge/data/`` (pitfall #11 -- higher layers never open a
...
def _paired_agreement_over(
    ...
    *,
    ...
    from mriforge.data.io_strategies import FastMRIH5Strategy, NiftiStrategy
    ...
    *,
    ...
    manifest.write_text(
        json.dumps(
            {
                "manifest_version": "4",
                "generator": "mriforge.infrastructure.training.strategies."
