...
class ReportCaseRecorder:
    ...
    def _metric_value(self, metrics: dict) -> float | None:
        ...
        from mriforge.core.metrics.metric_directions import _MONITOR_PREFIXES
    ...
    def write(self, run_dir: str | Path, subdir: str = "report_cases") -> Path:
        ...
        for k, (rank, case) in enumerate(self._select()):
            ...
            np.savez_compressed(out / npz_name, **case["arrays"])
            index.append(
                {
        ...
        (out / "cases_index.json").write_text(json.dumps(index, indent=2, sort_keys=True))
