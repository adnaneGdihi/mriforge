...
is installed (``pip install mriforge[viz]``) and fall back to the static
...
def _static_fallback(title: str, fig_id: str, figures: dict, *, note: bool = True) -> str | None:
    ...
    note_html = (
        (
            ...
            "(<code>pip install mriforge[viz]</code>) for the interactive "
            "version.</div>"
        )
        if note
        else ""
...
def build_html_report(
    ...
    *,
    ...
    body = (
        "<main>" + js + "".join(sections) + "</main><footer>Generated by mriforge report · "
        "QC</footer>"
    )
    doc = (
