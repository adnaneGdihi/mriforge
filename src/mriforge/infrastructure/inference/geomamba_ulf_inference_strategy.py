...
Companion to
:class:`mriforge.infrastructure.training.strategies.geomamba_ulf_strategy.GeoMambaULFStrategy`
...
Used by ``python -m mriforge.cli predict --config <yaml> --model
...
class GeoMambaULFInferenceStrategy(BaseInferenceStrategy):
    ...
    Args:
        model: a trained
            :class:`mriforge.models.generators.geo_mamba_unet.GeoMambaUNet`
        ...
        config: inference-time config; recognised keys:
            ...
            - ``apply_unwarp``: bool — when True and a ``b0_hz``
              ...
              :class:`mriforge.infrastructure.physics.unwarp.PhysicalUnwarp`
