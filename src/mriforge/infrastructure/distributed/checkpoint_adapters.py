...
3. **What the artifact IS.** DeepSpeed writes a tag *directory* of per-rank
   ...
   the campaign orchestrator, ``mriforge infer``) expects a single file, so the
...
class DefaultCheckpointAdapter:
    ...
    Prefix stripping is handled by :mod:`mriforge.core.module_utils` at the
...
class DeepSpeedCheckpointAdapter:
    ...
    (``discover_best_checkpoint``, campaign evaluation, ``mriforge infer``)
    ...
    def _save_consolidated(
        ...
        try:
            ...
            logger.error(
                ...
                "and `mriforge infer` will not find a file for this run.",
                exc,
                tag,
            )
            return None
...
def resolve_checkpoint_adapter(parallel: Any) -> IParallelCheckpointAdapter:
    ...
    ``deepspeed.save_consolidated_best``: that knob is a mriforge concern, not a
