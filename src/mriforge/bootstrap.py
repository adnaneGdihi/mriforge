...
from mriforge.infrastructure.logging import (
...
def _validate_data_availability_at_startup(config: TrainingSettings) -> None:
    ...
    try:
        ...
        # (src/mriforge/data/builders/manifest_index.py::build_index_from_manifest) —
        # so the pre-flight and the loader agree. The old raw ``Path(data_root).exists()``
        # plus an ad-hoc ``MRIFORGE_DATA_ROOT/data_root`` join diverged from that rule:
        # it raised "Data root not found" for a config whose data the loader would have
        # located via PathResolver's PROJECT_ROOT / MRIFORGE_DATA_ROOT / legacy-prefix
        ...
        if not Path(resolved).exists():
            raise ValueError(
                ...
                f"Set MRIFORGE_DATA_ROOT / PROJECT_ROOT to your data directory, or check "
...
def build_container(
    ...
    **Device**: resolved via ``mriforge.core.compute_device.resolve_torch_device``.
    ...
    # `mriforge audit` has always run. The audit and the run were checking
    ...
    # (mriforge.core.compute_device): an unset device means "auto", and "auto"
    ...
    # MetricsTracker delegates metric computation to mriforge.core.metrics
    save_images = False
    ...
    # F11.
    from mriforge.infrastructure.domain_validation import verify_startup_loss_domains
