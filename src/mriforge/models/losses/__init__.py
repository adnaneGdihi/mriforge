...
This module provides unified access to all loss functions via the registry pattern:
    from mriforge.models.losses import create_loss, create_composite_loss, register_loss

Phase 5 composition classes (recommended for composite losses):
    from mriforge.models.losses import ComposedLoss, WeightedLoss, ConditionalComposedLoss
"""
...
# MRIFORGE_PLUGINS modules. Placed at module end (late import, like the metrics
# subpackage import) so it never reorders the load-bearing ``from . import
# <loss>`` registration block above. mriforge.plugins is stdlib-only — no layer
# violation. A bad MRIFORGE_PLUGINS token raises (fail-fast, pitfall #15).
from mriforge.plugins import discover_plugins as _discover_plugins

_discover_plugins("mriforge.losses")
