...
class KALNConvNDLayer(nn.Module):
    ...
    def compute_legendre_polynomials(self, x, order):
        ...
        Returns:
            Any: Description.
        """
        P0 = torch.ones_like(x)  # P0 = 1 for all x, inherits device and dtype
        if order == 0:
    ...
    def forward_kal(self, x, group_index):
        ...
        # The original implementation was unsafe (division by zero) and coupled batch instances.
        # This is a safer, per-instance normalization.
        min_vals = torch.amin(x, dim=tuple(range(1, x.ndim)), keepdim=True)
        max_vals = torch.amax(x, dim=tuple(range(1, x.ndim)), keepdim=True)
    ...
    def forward(self, x):
        ...
        forward method for KALNConvNDLayer.
