...
class KAGNConvNDLayer(nn.Module):
        ...
        self.layer_norm = nn.ModuleList(
            [
                norm_class(
                    output_dim // groups,
                    output_dim,
                    **{k: v for k, v in norm_kwargs.items() if k != "bias"},
                )
                for _ in range(groups)
            ],
        )
    ...
    def forward(self, x):
        ...
        forward method for KAGNConvNDLayer.
