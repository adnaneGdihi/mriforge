...
(``mriforge.config.schemas.enums.DiffusionSampler.COLD_MRI``), but no
...
:class:`~mriforge.models.diffusion.kspace_process.PhysicsInformedColdDiffusion`
and forwards ``sample`` calls to it. The pattern mirrors
:class:`~mriforge.models.diffusion.pula_sampler.DPSMRISampler`.
"""
...
class ColdMRISampler(nn.Module):
    ...
    .. note::
       ...
       Only :class:`~mriforge.models.generators.kspace_cold_diffusion_generator.KSpaceColdDiffusionGenerator`
    ...
    def __init__(
        ...
        *,
        ...
        hard filter on what ``get_sampler`` can forward -- which is exactly why
        ``sampler_sigma`` / ``sampler_seed`` / ``selection_rule`` silently do not
        reach the core sampler on this path (issue #1286, tracked separately;
