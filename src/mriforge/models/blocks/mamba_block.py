...
def _mamba_fallback_allowed() -> bool:
    ...
    Opt-in ONLY, via ``MRIFORGE_ALLOW_MAMBA_FALLBACK`` — intended for CPU/CI
    ...
    label (pitfall #16).
    """
    return os.environ.get("MRIFORGE_ALLOW_MAMBA_FALLBACK", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
        ...
        **kwargs,
        ...
        try:
            ...
            try:
                ...
                # explicit MRIFORGE_ALLOW_MAMBA_FALLBACK opt-in (CPU/CI wiring).
                ...
                if not _mamba_fallback_allowed():
                    raise ImportError(
                        ...
                        f"MRIFORGE_ALLOW_MAMBA_FALLBACK=1 ONLY for CPU/CI wiring tests "
                        f"(such runs are not scientifically 'Mamba')."
                    ) from e2

                logger.warning(
                    "[MambaBlock] %s Falling back to Gated-Conv+GRU (NOT equivalent "
                    "to an SSM) because MRIFORGE_ALLOW_MAMBA_FALLBACK is set — wiring "
