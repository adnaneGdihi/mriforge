...
Get from `pip install` to a working MRIForge process in under five minutes.
...
```python
import mriforge
print(mriforge.__version__)  # 0.1.0
```
...
jobs with `export MRIFORGE_SUPPRESS_CLINICAL_WARNING=1`.
